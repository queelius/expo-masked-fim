...
We suppose there is some process that, informed by some degree of knowledge about the components, generates random candidate sets $\rvcand$ that \emph{plausibly} failed as depicted in \cref{fig:true_model}.
Note that, since our parametric model has a true parameter $\tparam\sysparam$, the candidate set must be some function of $\tparam\sysparam$ so that it may offer information about the true parameter index.
\begin{figure}
	\label{fig:gen_model}
...
\begin{definition}
A random $\alpha$-candidate set is a random candidate set $\rvcand$ in which the probability the failed component is included in the set of components is $\alpha \cdot 100 \%$.
\end{definition}

The \emph{cardinality} of a component set could theoretically convey information about the system. As a simplification, we make the following 
assumption.
\begin{assumption}
	\label{asm:indep_W_S}
...
The \emph{probability space} of this probabilistic model is given by the triple $\left((0,\infty) \times \PS{\Set{K}}, \Set{B} \times \PowerSet\left(\PS{\Set{K}}\right), \operatorname{P}\right)$ where $\Set{B}$ is the \emph{Borel space}, the set of half-open intervals, and $\operatorname{P}$ is the probability measure.
...
\begin{figure}
	...
	\caption{Graphical model of random variables where $\sv = \min\{\cv_1,\cv_2\ldots,\cv_m\}$, $\rvcand$ is a random $\alpha$-candidate set, $\RV{K}$ is the random component failure, and $\RV{W}$ is the cardinality of random candidate sets.}
	\begin{center}
		\input{img/graph_model_simplified.tex}
	\end{center}
\end{figure}
