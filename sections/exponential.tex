...
\begin{document}
\chapter{Expoentially distributed components}
\label{sec:common_distributions}

We consider series systems in which component lifetimes are exponentially 
distributed and compare them to the constrained maximum entropy distribution, 
which consists of identical exponentially distributed component lifetimes. 

\section{Parametric functions}
Suppose the \jth component has an exponentially distributed lifetime and a 
failure rate $\tparam{\lambda}_j$, denoted by 
...
An $m$-out-of-$m$ system with exponentially distributed lifetimes is also 
exponentially distributed as proved by the following theorem.
\begin{theorem}
...
Knowing the distribution of component lifetimes has many important 
applications. The canonical use-case is, if a failure occurs in a series system 
at time $t$, for any component we know the probability that it is the component 
cause of failure. However, there are other applications as well. For instance, 
a system designer may desire a system where the component cause of failure is 
as uncertain as possible to prevent an \emph{adversary} from learning about 
...
\subsubsection{Two candidates} If we condition on samples in which each 
observation consists of two candidates, $\rv{W} = \cardinality{\rvcand} = 2$, 
