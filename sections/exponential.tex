...
\begin{document}
\chapter{Case study: exponentially distributed component lifetimes}
\label{sec:common_distributions}
Consider a series system in which the component lifetimes are exponentially distributed. It is a popular choice due to its analytical tractability, e.g., it is the only continuous distribution with a constant failure rate.

\section{System distribution}
Consider component $j$ with an exponentially distributed lifetime and a failure rate $\tparam{\lambda}_j$, denoted by $\cv_j \sim \text{EXP}(\tparam{\lambda}_j)$. The random variable $\cv_j$ has reliability, density, and failure rate functions given respectively by
\begin{align}
...
A series system composed of $m$ components with exponentially distributed lifetimes, $\cv_j \sim \text{EXP}(\tparam{\lambda}_j)$ for $j=1,\ldots,m$, has a random lifetime denoted by the random variable $\sv = \min(\cv_1,\ldots,\cv_m)$ with a true parameter value given by
\begin{equation}
    \tparam{\vec\lambda} = \veclist{\tparam\lambda_1, \ldots, \tparam\lambda_m}^{\intercal}\,,
\end{equation}
where $\cv_j$ has a true parameter value indexed by element $j$ of $\tparam{\vec\lambda}$. The system's reliability is given by
\begin{equation}
\label{eq:sys_expo_reliability_function}
    \srv_{\sv} \left(t \given \tparam{\vec\lambda} \right) =
        ...
        \indicator{t > 0} + \indicator{t \leq 0}\,.\\
\end{equation}
\begin{proof}
By \cref{thm:series_reliability_function},
\begin{equation}
    \srv_{\sv}(t \given \tparam{\vec\lambda}) = \prod_{j=1}^{m} \srv_j(t \given \tparam\lambda_j)\,.
\end{equation}
Plugging in the component reliability functions as given by \cref{eq:expo_reliability} results in
\begin{equation}
    \srv_{\sv}(t \given \tparam{\vec\lambda}) = \prod_{j=1}^{m} \exp(-\tparam\lambda_j \cdot t) =
    \exp \left(-\left [ \sum_{j=1}^{m}{\tparam\lambda_j} \right ] t\right)\,.
\end{equation}
\end{proof}

The system's reliability function belongs to the family of exponential distributions with a failure rate that is the sum of the component failure rates.
\begin{theorem}
\label{thm:expo_series_family}
The random lifetime $\sv$ of a series system composed of $m$ components with exponentially distributed lifetimes is exponentially distributed with a failure rate that is the sum of the component failure rates. That is,
\begin{equation}
    \sv \sim \expdist \left(\sum_{j=1}^{m} \tparam{\lambda}_j\right)\,,
\end{equation}
where $\tparam{\lambda_j}$ is the failure rate of component $j$.
\end{theorem}

By \cref{def:expo_series_family}, the system has density and failure rate functions given respectively by
\begin{align}
\label{eq:expo_sys_pdf}
\spdf(t \given \tparam{\vec\lambda}) &=
    \left ( \sum_{j=1}^{m} {\tparam{\lambda}_j} \right ) \exp
    \left(-\left [ \sum_{j=1}^{m}{\tparam{\lambda_j}} \right ] t\right)\indicator{t > 0}\,,\\
...
\label{eq:expo_prob_K_given_S}
    \pmf_{\rv{K} \given \sv}(k \given t, \tparam{\vec\lambda}) = \pmf_{\rv{K}}(k \given \tparam{\vec\lambda}) =
        \frac{\tparam{\lambda}_k}{\sum_{p=1}^{m} \tparam{\lambda}_p}\indicator{k \in \{1, \ldots, m\}}\,.
\end{equation}
\begin{proof}
...
\pdf_{\rv{K}, \sv}(k, t \given \tparam{\vec\lambda}) =
    ...
    \indicator{t > 0}\indicator{k \in \{1, \ldots, m\}}
\end{equation}
\begin{proof}
By definition,
\begin{equation}
...
\end{align}
\end{proof}
The marginal distribution of $\rvcand \given \rv{W} = w$ given by \cref{} is the same as the joint distribution of $\rvcand, \sv \given \rv{W} = w$ given by \cref{eq:expo_cand_given_s}. Thus, $\rvcand$ and $\sv$ are conditionally independent given $\rv{W} = w$.
...
\end{equation}
where the summation indexed by $\cand$ is over $\powerset(\{1,\ldots,m\})$.
\begin{proof}
...
A $(1-\alpha) \cdot 100 \%$-confidence interval for $\tparam{\lambda}_j$ is given by
\begin{equation}
...
\begin{equation}
    \operatorname{\expectation_{\vec{\tparam\lambda}}}[\sv] = \frac{1}{\sum_{p=1}^{m} \tparam\lambda_j}\,.
\end{equation}
The maximum likelihood estimator of $\operatorname{\expectation_{\vec{\tparam\lambda}}}[\sv]$ is given by $\operatorname{\expectation_{\expomle}}[\sv]$, which has an asymptotic sampling distribution given by
\begin{equation}
    \rv{S}_{\operatorname{\expectation_{\expomle}}[\sv]} \sim \mathcal{N}\left(\operatorname{\expectation_{\vec{\tparam\lambda}}}[\sv], \var\left[\operatorname{\expectation_{\expomle}}[\sv]\right]\right)\,,
\end{equation}
where
\begin{equation}
    \var\left[\operatorname{\expectation_{\expomle}}[\sv]\right] = a\,.
\end{equation}

If a system failure occurs, the expected number of inspections is given by
\begin{equation}
    \operatorname{\expectation_{\vec{\tparam\lambda}}}[\rv{N}] =
    \frac{\sum_{n=1}^{m} n \cdot \tparam{\lambda}_{\orderfunc(n)}}{\sum_{p=1}^{m} \tparam{\lambda}_p}\,,
\end{equation}
where $\orderfunc(j) \leq \orderfunc(k) \implies \tparam\lambda_j \geq \tparam\lambda_k$. The maximum likelihood estimator of $\operatorname{\expectation_{\vec{\tparam\lambda}}}[\rv{N}]$ is given by $\operatorname{\expectation_{\expomle}}[\rv{N}]$, which has an asymptotic sampling distribution given by
\begin{equation}
    \rv{S}_{\operatorname{\expectation_{\expomle}}[\rv{N}]} \sim \mathcal{N}\left(\operatorname{\expectation_{\vec{\tparam\lambda}}}[\rv{N}], \var\left[\operatorname{\expectation_{\expomle}}[\rv{N}]\right]\right)\,,
\end{equation}
where an estimator of the variance-covariance $\var\left[\operatorname{\expectation_{\expomle}}[\rv{N}]\right]$ is described in \cref{sec:app:monte_carlo}.

\section{3-out-of-3 system}
...
\paragraph{Two candidates} If we condition on samples in which each observation consists of two candidates, $\rv{W} = \cardinality{\rvcand} = 2$, the likelihood function is given by
\begin{equation}
\begin{split}
...
\subsubsection*{Three candidates}
...
\begin{tikzpicture}[scale=4]
    ...
    \draw[->] (0,0,0) -- (1,1,1) node[above right] {$\vec{n} = \veclist{1,1,1}^\intercal$};
...
In this section, we evaluate how rapidly the sampling distribution of $\mle$ converges to the asymptotic distribution as given by \cref{}, a multivariate normal with a mean $\tparam\sysparamvec$ and a variance-covariance matrix $\frac{1}{n} \infomatrx^{-1}\left(\tparam\sysparamvec\right)$.
...
\begin{equation}
\estimator{\mse}\left(\mlei{1}, \ldots, \mlei{r}\right) = \frac{1}{r} \sum_{i=1}^{r} \left( \mlei{i} - \tparam\sysparamvec \right)^\intercal \left( \mlei{i} - \tparam\sysparamvec \right)\,.
\end{equation}
...
---
Let's use a profile likelihood to look at how the likelihood function changes with respect to a parameter component. Say, for instance, the other cmoponents in the parameter are nuisance parameters, and we are really just interested in the \jth component.
---



Let $\tparam{\vec\lambda} = \veclist{1,2,3}^\intercal$.

and let the masked failure sample be of size $n=1000$. Theoretically, the MLE $\vec{\estimator{\vec\lambda}_{1000}}$ is approximately normally distributed,
\begin{equation}
\label{eq:expo_mvn_2_3_4}
    \vec{\estimator{\lambda}_{1000}} \sim \mathrm{MVN} \bigg ( \veclist{2, 3, 4}^\intercal, \covn{1000}\left(???\right) \bigg )\,.
\end{equation}
...
A sample variance-covariance matrix of $r=10000$ maximum likelihood estimates, $\expomlei{1}{1000}, \ldots, \expomlei{10000}{1000}$, is computed to be
\begin{equation}
    \cov{1000} =
    \left(
        \begin{array}{ccc}
...
\begin{align}
    \lVert \matrx{C} \rVert_F
        &= \sqrt{\trace \left( \matrx{C} \matrx{C^\intercal}\right)}\\
        &= \sqrt{\vt{\matrx{A}}^\intercal \vt{\matrx{A}}}\,.
\end{align}
...
\begin{equation}
    n(\mle - \tparam\sysparamvec)^\intercal \infomatrx(\tparam\sysparamvec) (\mle - \tparam\sysparamvec) \sim \chi_{m \cdot q}^2\,.
\end{equation}
