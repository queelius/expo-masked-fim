...
\label{sec:app}
Suppose we have a characteristic of interest that is a function of $\tparam\sysparamvec \in \mathbb{R}^{m \cdot q}$, denoted by $\vecstat(\tparam\sysparamvec) \colon \mathbb{R}^{m \cdot q} \mapsto \mathbb{R}^{p}$. By the invariance property of maximum likelihood estimators, if $\mleu$ is the maximum likelihood estimator of $\tparam\sysparamvec$ then $\vecstat(\mleu)$ is the maximum likelihood estimator of $\vecstat(\tparam\sysparamvec)$.
...
Asymptotically, the random variable $\left[\sampdu\right]_j$ realizes value less than constant $a$ with probability given by
\begin{equation}
    \prob\left[\left[\sampdu\right]_j < a \right] = \operatorname{\Phi}\!\left(\frac{a - \tparam\sysparamvec_j}{\sigma_j}\right)\,,
\end{equation}
...
\label{eq:prob_interval}
     \tparam\sysparamvec_j \, \raisebox{.2ex}{$\scriptstyle\pm$} \, \sigma_j \operatorname{\Phi^{-1}}\!\left(p/2\right)\,,
\end{equation}
...
An asymptotic $(1 - \alpha) \cdot 100 \%$ confidence interval for the \jth component of the true parameter index $\tparam\sysparamvec$ is given by
\begin{equation}
...
Applying \cref{eq:hypothesis_test_mean} to the marginal of $\left[\sampd\right]_j$ generates the confidence interval described in \cref{}.

\section{Monte-carlo simulation}
\label{sec:app:monte_carlo}
The generative model of the sampling distribution of $\sampdu$, as described in \cref{alg:samp_du_gen_model}, is asymptotically equivalent to generating samples from the multivariate normal distribution given by \cref{}. However, generating samples from the multivariate normal is much less computationally demanding than the non-asymptotic model.
...
\end{equation}
where $\jacobian\left(\,\cdot\,\right) \in \mathbb{R}^{?}$ is the Jacobian as given by \cref{}.
