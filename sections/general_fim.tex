...
\begin{definition}
Suppose $\RV{X}$ is a discrete random variable with a support $\Set{X}$.
The \emph{entropy} of $\RV{X}$ is a measure of uncertainty defined as
\begin{equation}
	\entropy(\RV{X}) \coloneqq -\sum_{x \in \Set{X}} \pmf_{\RV{X}}(x)
	\log_2 \pmf_{\RV{X}}(x)\,.
\end{equation}
The \emph{differential} entropy of the \emph{continous} random variable $\RV{X}$ with a support $\Set{X}$ takes a similar form defined as
\begin{equation}
\entropy(\RV{X}) \coloneqq -\int_{\Set{X}} \pdf_{\RV{X}}(x) \log_2 \pdf_{\RV{X}}(x) \dif t\,.
\end{equation}
\end{definition}

For example, if a diagnostician could ask ``yes'' or ``no'' questions about the component cause of failure, an \emph{expected lower-bound} on the number of questions required to determine which component caused the failure is given by $\entropy(\RV{K})$.
The greater the entropy, the greater the number of questions the diagnostician needs on average to determine the component cause of failure.

The joint entropy of $\RV{X}$ and $\RV{Y}$ is defined as
\begin{equation}
	\entropy(\RV{X}, \RV{Y}) \coloneqq -\!\! \sum_{x \in \Set{X}, y \in \Set{Y}}
	\pdf_{\RV{X},\RV{Y}}(x,y) \log_2 \pdf_{\RV{X},\RV{Y}}(x,y)\,.
\end{equation}

Suppose we are interested in the conditional entropy of $\RV{X}$ given $\RV{Y} = y$,
\begin{equation}
\entropy(\RV{X} \Given \RV{Y} = y) \coloneqq - \sum_{x \in \Set{X}}
\pdf_{\RV{X} \Given \RV{Y}}(x \Given y) \log_2 \pdf_{\RV{X} \Given \RV{Y}}(x \Given y)\,.
\end{equation}

If we \emph{average} over all of the values $\RV{Y}$ may realize, we get the conditional entropy
\begin{align}
	\entropy(\RV{X} \Given \RV{Y}) &\coloneqq \sum_{y \in \Set{Y}} \pdf_{\RV{Y}}(y) \entropy(\RV{X} \Given \RV{Y} = y)\\
...
That is, observing one of the random variables does not reduce the uncertainty of the other random variable..

For example, if at least one of the components in the series system has a different lifetime distribution than the rest, then observing a system failure time \emph{reduces} the uncertainty about the component cause of failure, i.e., $\entropy(\RV{K} \Given \sv) \leq \entropy(\RV{K})$.

When we make an \emph{observation} of a random variable, the entropy is a measure of the \emph{expected information} provided by the observation.
Intuitively, the more uncertain the outcome, the greater the information.
...
Given some distribution $\sv$, the \emph{maximum} entropy is when the failed component is in the candidate set by random chance, $\RV{A} = \frac{w}{m}$, as givne by
\begin{equation}
...
By \cref{def:msft_joint_pdf}, the probability density that $\mfs = \Tuple{\cand_1,t_1},\ldots,\Tuple{\cand_n,t_n}$ conditioned on $\RV{W} = w$ is given by
\begin{equation*}
\begin{split}
& \PDF{\Tuple{\cand_1,t_1},\ldots,\Tuple{\cand_n,t_n} \Given w, \tparam{\sysparamvec}}[\mfs \Given \RV{W}] =\\
...
\begin{equation}
    \likelihood_n(\sysparamvec \Given w) = \prod_{i=1}^{n} \jointpdf(\cand_i, t_i \Given w, \sysparamvec)\,.
\end{equation}
\end{definition}
...
The log-likelihood with respect to $\sysparamvec$ given a particular $\mfs = \Tuple{\cand_1,t_1},\ldots,\Tuple{\cand_n,t_n}$ conditioned on $\RV{W} = w$ is given by
\begin{equation}
\label{eq:log_like_mle}
\Fun{\ell_n}(\sysparamvec \Given w) = \sum_{i=1}^{n} \ln \jointpdf(\cand_i, t_i \Given w, \sysparamvec)\,.
\end{equation}
\end{definition}
...
\begin{equation}
    \nabla \Fun{g}\left(x_1,\ldots,x_q\right) = \left (
        \pd{\Fun{g}(x_1,\ldots,x_q)}{x_1},\,
        \ldots\,,
        \pd{\Fun{g}(x_1,\ldots,x_q)}{x_q}
    \right )^{\Transpose}\,.
\end{equation}
\end{definition}
...
The Fisher's score function, denoted by $\Fun{s_n} \colon \RealSet^{m \cdot q} \mapsto \RealSet^{m \cdot q}$, is the gradient of the log-likelihood function given a sample of size $n$,
\begin{equation}
\label{eq:score}
    \Fun{s_n}(\sysparamvec \Given w) \coloneqq \nabla \Fun{\ell_n}(\sysparamvec \Given w)\,.
\end{equation}
\end{definition}
...
\label{eq:covariance_matrix_op}
    ...
    \left [
        ...
        \left(\RV{\vec{X}} - \expectation[\RV{\vec{X}}] \right)^\Transpose
    \right ]\,,
\end{equation}
...
\label{eq:info_matrix_var}
    \infomatrxn\left(\tparam\sysparamvec \Given w \right)
        = \operatorname{\var_{\tparam\sysparamvec}}\!\!\left[\score_n\left(\tparam\sysparamvec \Given w\right)\right]\,,
\end{equation}
...
= -\sum_{\cand \in \candsw{w}} \int_{0}^{\infty}
{
	...
	\ln \pdf_{\rvcand,\sv \Given \RV{W},\RV{A}}(\cand, t \Given w, \alpha, \tparam\sysparamvec)
	\biggr]
}
