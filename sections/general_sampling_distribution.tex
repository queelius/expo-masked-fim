...
\begin{equation}
\label{eq:crlb}
    \matrx{\rm{CRLB}_{n}} = \frac{1}{n} \infomatrx^{-1}\left(\tparam\sysparamvec \right)\,.
\end{equation}
...
By \cref{eq:mse_unbiased,eq:crlb}, the mean squared error of any unbiased point estimator that is only a function of $\mfs$ has a lower-bound given by the trace of $\matrx{\rm{CRLB}_{n}}$,
\begin{equation}
\label{eq:mse_lower_bound_total}
\mse\left(\rv{Y}\right) \geq \trace\left(\matrx{\rm{CRLB}_{n}}\right)\,.
\end{equation}
...
A parameter index $\mle$ that maximizes the \emph{likelihood} of observing the given $\mfsw = \mfso$ (see \cref{def:msfs}) is a maximum likelihood estimator of $\tparam\sysparamvec$. That is,
\begin{equation}
...
The accuracy of the maximum likelihood estimator is a function of how indicative candidate sets are. For instance, we assume that the candidates are generated from the joint probability density of $\sv$ and $\rvcand$. If this is not a very accurate model (e.g., the inspector who constructed the candidate sets in the sample preferred to check a subset of the components for failure, and tended to neglect the others), the maximum likelihood estimator will be inconsistent since the model is not consistent with reality. Rather, the maximum likelihood estimator will generate a maximum likelihood estimate that will make the provided sample the most likely to be generated by the assumed model, i.e., the component lifetimes will not be accurately modeled since they must be modeled in such a way as to make the ``biased'' sample more likely.
...
\label{def:asym_covariance_matrix}
    ...
    \left[
        ...
        \left(\sampd - \tparam\sysparamvec \right)^\intercal
    \right]\,.
\end{equation}
...
\mse \left( \sampd \right) = \frac{1}{n} \trace \left ( \infomatrx^{-1} \bigl(\tparam\sysparamvec \given w\bigr) \right) = \trace\left(\matrx{\rm{CRLB}_{n\,w}}\right)\,.
\end{equation}
...
Suppose that the estimates are given by the maximum likelihood estimator described in \cref{}. The maximum likelihood estimator given these estimates has a sampling distribution given by the following definition.
\begin{definition}
...
\begin{align}
    \var\left[\sampdu\right]
        ...
        &= \matrx{B} \var\left[\sum_{i=1}^{r} \matrx{A_i} \sampdir{n_i}{w_i}{i} \right] \matrx{B^\intercal}\\
        &= \matrx{B} \left(\sum_{i=1}^{r} \var\left[\matrx{A_i} \sampdir{n_i}{w_i}{i} \right]\right) \matrx{B^\intercal}\\
        &= \matrx{B} \left(\sum_{i=1}^{r} \matrx{A_i} \var\left[ \sampdir{n_i}{w_i}{i} \right] \matrx{A_i^\intercal} \right) \matrx{B^\intercal}\,.
\end{align}
...
\begin{align}
    \var\left[\sampdu\right]
        &= \matrx{B} \left(\sum_{i=1}^{r} \matrx{A_i} \matrx{A_i^{-1}} \matrx{A_i^\intercal} \right) \matrx{B^\intercal}\\
        &= \matrx{B} \left(\sum_{i=1}^{r} \matrx{A_i^\intercal}\right) \matrx{B^\intercal}\,.
\end{align}
The summation is equivalent to $\matrx{\left({B^\intercal}\right)^{-1}}$. Performing this substitution results in
\begin{align}
    \var\left[\sampdu\right]
        &= \matrx{B} \matrx{\left(B^\intercal\right)^{-1}} \matrx{B^\intercal}\\
        &= \matrx{B}
\end{align}
...
\end{equation}
If only one maximum likelihood estimate is realized, $\sampdu = \mleu$, then the sample covariance given by \cref{eq:sample_var_sampdu} cannot be computed. In the \emph{parametric Bootstrap}, the sample covariance is approximated by using $\mleu$ as an estimate of $\tparam\sysparamvec$ in the generative model described by \cref{}, i.e.,
\begin{equation}
