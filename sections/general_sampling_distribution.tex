...
\mse(\sampdpt) = \trace\!\left( \var\left(\sampdpt\right) \right)\,.
\end{equation}
...
Suppose the only information about the true parameter index $\tparam\sysparamvec$ is given by a sample of $n$ masked system failure times in which only those system failures with $w$ candidates are observed.
...
A parameter index $\mle$ that maximizes the \emph{likelihood} of observing some realization of $\mfs$ given $\RV{W} = w$ (see \cref{def:msfs}) is a maximum likelihood estimator of $\tparam\sysparamvec$.
That is,
\begin{equation}
...
\begin{definition}
Let $\mfs$ be a random sample of $n$ masked system failure times in which $n_i$ realizations have $w_i$ $\alpha_i$-masked candidate sets for $i = 1,\ldots,r$.
...
\begin{align}
    ...
    &= \expectation\left[\left( \sum_{i=1}^{r} \matrx{A_i} \right)^{-1} \left( \sum_{i=1}^{r} \matrx{A_i} \sampdir{n_i}{w_i,\alpha_i}{i} \right)\right]\\
    &=  \left( \sum_{i=1}^{r} \matrx{A_i} \right)^{-1}
            \left( \sum_{i=1}^{r} \matrx{A_i}
            \expectation\left[\sampdir{n_i}{w_i,\alpha_i}{i}\right]\right)\\    
...
\label{eq:var_samp_d_full}
    \var\left[\sampdu\right] = \left( \sum_{i=1}^{r} n_i \infomatrx(\tparam\sysparamvec \Given w_i, \alpha_i) \right)^{-1}\,.
\end{equation}
\end{theorem}
\begin{proof}
Let
...
\begin{align}
    \var\left[\sampdu\right]
        &= \var\left[\matrx{B} \left( \sum_{i=1}^{r} \matrx{A_i} \sampdir{n_i}{w_i,\alpha_i}{i} \right)\right]\\
        &= \matrx{B} \var\left[\sum_{i=1}^{r} \matrx{A_i} \sampdir{n_i}{w_i,\alpha_i}{i} 
        \right] \matrx{B^{\Transpose}}\\
        &= \matrx{B} \left(\sum_{i=1}^{r} \var\left[\matrx{A_i} 
        \sampdir{n_i}{w_i,\alpha_i}{i} \right]\right) \matrx{B^{\Transpose}}\\
        &= \matrx{B} \left(\sum_{i=1}^{r} \matrx{A_i} \var\left[ 
        \sampdir{n_i}{w_i,\alpha_i}{i} \right] \matrx{A_i^{\Transpose}} \right) 
        \matrx{B^{\Transpose}}\,.
\end{align}
...
\begin{algorithm}
    ...
    \func{$\genSampleMLE${$(\tparam\sysparam)$}}{
        draw $\alpha \sim \pmf_{\RV{A}}(\, \cdot \,)$\;        
        draw candidate cardinality $w \sim \pmf_{\RV{W} \Given \RV{A}}(\, \Given \alpha \,)$\;
