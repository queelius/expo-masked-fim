...
The joint probability density function of $\rvcand$ and $\sv$ is given by the following theorem.
\begin{theorem}
	\begin{equation}
	\pdf_{\rvcand,\sv \given \RV{W}}(\cand,t \given w, \alpha, \tparam\sysparam) = \sum_{k=1}^{m} \pdf_{\rvcand,\RV{K},\sv \given \RV{W}}(\cand,k,t \given w, \alpha)
	\end{equation}
...
The \emph{optimal} mask is provided by an $\alpha=1.0$ masked candidate set as given by the following corollary.
\begin{corollary}
	\label{thm:general_joint_likelihood_cand}
	The joint probability density of $\rvcand$ and $\sv$ conditioned on $\RV{W} = w$ and $\alpha=1.0$ is given by
	\begin{equation}
	\label{eq:general_joint_likelihood_cand}
	\jointpdf(\cand, t \given w, \alpha=1.0,\tparam\sysparam) = \frac{\srv_{\sv}(t \given \tparam\sysparam)}{\binom{m-1}{w-1}}	
