...
\section{Candidate $\alpha$-sets}
\label{sec:cand_sel}
The series system consists of $m$ components with random lifetimes $\cv_1,\ldots,\cv_m$.
These random variables are \emph{unobservable} and thus, for instance, we cannot know with certainty from a sample of masked system failures which component failed.

The $\alpha$-candidate sets have a conditional probability distribution given by the following theorem.
\begin{theorem}
	The random $\alpha$-candidate set $\rvcand$ conditioned on $\RV{W} = w$ and $\RV{K} = k$ has a probability distribution given by
	\begin{equation}
	\pdf_{\rvcand \given \RV{K}, \RV{W}}(\cand \given k, w, \alpha) = \frac{\alpha}{\binom{m-1}{w-1}} \SetIndicator{\cand}(k) + \frac{1-\alpha}{\binom{m-1}{w}} \SetIndicator{\SetComplement[\cand]}(k)
	\end{equation}
...
\begin{remark}
The $\alpha$-candidate set model is somewhat naive since $\pdf_{\rvcand \given \RV{K}, \RV{W}}(\Set{A} \given k, w, \alpha) = \pdf_{\rvcand \given \RV{K}, \RV{W}}(\Set{B} \given k, w, \alpha)$ if $\SetIndicator{\Set{A}}(k) = \SetIndicator{\Set{B}}(k)$, e.g., $\alpha$-candidate sets that include the failed component are equally likely to be chosen.
A more informative generative model may preferentially generate candidate sets whose components are about to fail as well.
\end{remark}

The random variable $\RV{K}$ is \emph{latent}, or unobservable.
In a masked system failure sample, the only observable is the candidate set and the system failure time.
The joint probability density function of $\rvcand$ and $\sv$ conditioned on $\RV{W}$ is given by the following theorem.
\begin{theorem}
The conditional joint probability density function of random variable $\rvcand$ and random system lifetime $\sv$ given $\RV{W} = w$ is given by
	\begin{equation}
	\pdf_{\rvcand,\sv \given \RV{W}}(\cand,t \given w, \alpha, \tparam\sysparam) =
		\sum_{k=1}^{m} \pdf_{\rvcand \given \RV{K}, \RV{W}}(\cand \given k, w, \alpha) \pdf_{\RV{K},\sv}(k,t \given \tparam\sysparam)
	\end{equation}
	with a sample space $\candsw{w} \times (0,\infty)$.
\end{theorem}
\begin{proof}
By the laws of probability, the joint distribution of $\rvcand$, $\RV{K}$ and $\sv$ conditioned on $\RV{W} = w$ is given by
\begin{equation}
	\pdf_{\rvcand,\RV{K},\sv \given \RV{W}}(\cand,k,t \given w, \alpha, \tparam\sysparam) = \pdf_{\rvcand \given \RV{K}, \RV{W}}(\cand \given k, w, \alpha, \tparam\sysparam) \pdf_{\RV{K},\sv \given \RV{W}}(k,t \given w,\tparam\sysparam)\,.
\end{equation}
However, $\RV{K}$ and $\sv$ are independent of $\RV{W}$ and $\rvcand$ conditioned on $\RV{K}$ and $\RV{W}$ is independent of $\tparam\sysparam$, so we simplify the joint probability density function to
\begin{equation}
...
\begin{corollary}
	...
	\jointpdf(\cand, t \given w, \alpha=1,\tparam\sysparam) = \frac{\srv_{\sv}(t \given \tparam\sysparam)}{\binom{m-1}{w-1}}	
...
We claim without proof the joint probability $\jointpdf(\cand, t \given w, \alpha=\frac{w}{m},\tparam\sysparam)$ is proportional to $\spdf(t \given \tparam\sysparam)$ and therefore, as intuition suggests, candidate sets that are randomly chosen carry no information about the true parameter index.
...
If it is known or estimated that the candidate sets are misinforming, then we may estimate the answer to a related but different question, ''What is not the value of $\tparam\sysparam$?''
Granted, this is not nearly as informative, but the only case in which no information is conveyed is given by randomly generating the candidate sets.
