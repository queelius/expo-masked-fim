...
\begin{theorem}
	...
	\begin{equation}
	\pdf_{\rvcand \Given \RV{K}, \RV{W}, \RV{A}}(\cand \Given k, w, \alpha) = \frac{\alpha}{\binom{m-1}{w-1}} \SetIndicator{\cand}(k) + \frac{1-\alpha}{\binom{m-1}{w}} \SetIndicator{\SetComplement[\cand]}(k)
	\end{equation}
...
\pdf_{\rvcand,\sv \Given \RV{W}, \RV{A}}(\cand,t \Given w, \alpha, \tparam\sysparam) =
	\sum_{k=1}^{m} \pdf_{\rvcand \Given \RV{K}, \RV{W}, \RV{A}}(\cand \Given k, w, \alpha) \pdf_{\RV{K},\sv}(k,t \Given \tparam\sysparam)
\end{equation}
with a sample space $\candsw{w} \times (0,\infty)$.
\Cref{eq:joint_alpha} may be rewritten as
\begin{equation}
\label{eq:joint_alpha_2}
\mathsmaller{\pdf_{\rvcand,\sv \Given \RV{W}}(\cand,t \Given w, \alpha, \tparam\sysparam) =
...
\begin{equation}
	\pdf_{\rvcand,\RV{K},\sv \Given \RV{W}, \RV{A}}(\cand,k,t \Given w, \alpha, \tparam\sysparam) = \pdf_{\rvcand \Given \RV{K}, \RV{W}}(\cand \Given k, w, \alpha, \tparam\sysparam) \pdf_{\RV{K},\sv \Given \RV{W}}(k,t \Given w,\tparam\sysparam)\,.
\end{equation}
...
\begin{equation}
\pdf_{\rvcand,\RV{K},\sv \Given \RV{W}, \RV{A}}(\cand,k,t \Given w, \alpha, \tparam\sysparam) = \pdf_{\rvcand \Given \RV{K}, \RV{W}}(\cand \Given k, w, \alpha) \pdf_{\RV{K},\sv}(k,t \Given \tparam\sysparam)\,.
\end{equation}
...
\begin{corollary}
	...
	\frac{\SetIndicator{\candsw{w}}(\cand)}{\binom{m-1}{w}}
	\left(
	1-\alpha -	
	\frac
	{
		(1-\alpha \frac{m}{w}) \sum_{j \in \cand} \haz_j(t \Given \tparam\sysparam_j)
	}
	{
		\haz_{\sv}(t \Given \tparam\sysparam)
	}\right)\,,
