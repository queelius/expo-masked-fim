...
\begin{proposition}[Information Recovery under Deterministic Masking]
\label{prop:deterministic_masking}
For an $m$-component exponential series system with $1 \leq w \leq m-1$, suppose the masking mechanism is \emph{separating-deterministic}: there exists an injection $\varphi\colon \{1, \ldots, m\} \to \{\cand \subseteq \{1,\ldots,m\} : |\cand| = w\}$ satisfying $k \in \varphi(k)$ and $\Prob{\cand = \varphi(k) \mid \RV{K} = k} = 1$ for each~$k$. Then the Fisher information matrix equals that of complete identification ($w = 1$):
\begin{equation}
\label{eq:deterministic_fim}
[\infomatrx(\tparam{\vec{\lambda}} \mid \varphi)]_{jk} = \frac{\delta_{jk}}{\Lambda\, \tparam{\lambda}_j}
\end{equation}
where $\Lambda = \sum_{j=1}^m \tparam{\lambda}_j$ and $\delta_{jk}$ is the Kronecker delta. In particular, for $w = m-1$ the candidate sets $\{1,\ldots,m\} \setminus \{j\}$ are in bijection with the $m$ components, and any derangement $\sigma$ (a permutation with no fixed point) defines such a mechanism via $\varphi(k) = \{1,\ldots,m\} \setminus \{\sigma(k)\}$. Derangements exist for all $m \geq 2$.
\end{proposition}

\begin{proof}
Since $\varphi$ is injective, each candidate set in the image of~$\varphi$ is produced by exactly one component, so the map $\cand \mapsto \varphi^{-1}(\cand)$ recovers the failed component with certainty. Marginalizing the joint density~\eqref{eq:expo_joint} over the failure cause:
\[
\pdf(\ti, \cand \mid \vec{\lambda})
= \sum_{k=1}^{m} \tparam{\lambda}_k \, e^{-\Lambda \ti}\, \Prob{\cand = \cand \mid \RV{K} = k}
= \lambda_{\varphi^{-1}(\cand)}\, e^{-\Lambda \ti},
\]
where the second equality uses $\Prob{\cand = \cand \mid \RV{K} = k} = \SetIndicator{\{\varphi(k)\}}(\cand)$ and the injectivity of~$\varphi$. The log-density is thus
\[
\ln \pdf(\ti, \cand \mid \vec{\lambda}) = \ln \lambda_{\varphi^{-1}(\cand)} - \Lambda\, \ti,
\]
which has the same functional form as the unmasked log-density with identified cause $\RV{K} = \varphi^{-1}(\cand)$. The second partial derivative is $\partial^2 / \partial \lambda_j \partial \lambda_k = -\delta_{j,\varphi^{-1}(\cand)}\,\delta_{jk}/\lambda_j^2$, and taking the expectation over $(\sv, \cand)$ gives $[\infomatrx]_{jk} = \delta_{jk}\, \Prob{\RV{K}=j}/\lambda_j^2 = \delta_{jk}/(\Lambda\, \lambda_j)$, where the last step uses $\Prob{\RV{K}=j} = \lambda_j/\Lambda$ (Proposition~\ref{prop:expo_K_indep_S}).

For the existence claim: when $w = m-1$, the candidate sets $c_j = \{1,\ldots,m\} \setminus \{j\}$ satisfy $k \in c_j$ if and only if $j \neq k$, so the map $\varphi(k) = c_{\sigma(k)}$ satisfies $k \in \varphi(k)$ whenever $\sigma(k) \neq k$. The number of derangements of $\{1,\ldots,m\}$ is $D_m = m!\sum_{i=0}^{m}(-1)^i/i! \geq 1$ for $m \geq 2$.
\end{proof}

\begin{remark}
Proposition~\ref{prop:deterministic_masking} shows that the Fisher information loss from masking at cardinality~$w$ is not inherent to the cardinality, but depends on the masking mechanism. A well-designed diagnostic that always maps each failure cause to a unique candidate set incurs \emph{no information loss whatsoever}, even at $w = m-1$. The uniform masking model studied in this paper lies at the opposite extreme: it provides no preferential information about component identity, and the resulting FIM (Proposition~\ref{prop:expo_fisher}) is strictly smaller than the complete-identification FIM. Note that the separating-deterministic mechanism violates the symmetry condition C2 (see Remark~\ref{rem:c1c2c3_comparison}), since $\Prob{\cand = \varphi(k) \mid \RV{K}=k} = 1$ but $\Prob{\cand = \varphi(k) \mid \RV{K}=k'} = 0$ for $k' \neq k$ with $k' \in \varphi(k)$.
\end{remark}
