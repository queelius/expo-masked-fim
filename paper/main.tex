...
The closed-form results for $w=m-1$ are particularly significant because this is the \emph{maximally uninformative identifiable} masking configuration. The masking cardinality ranges from $w=1$ (exact cause identification, no masking) through $w=m$ (complete masking, non-identifiable). Each candidate set of size $w$ excludes $m-w$ components from suspicion, so larger $w$ means less diagnostic information per observation. At $w=m-1$, each observation excludes only a single component, retaining the minimum information needed for identifiability. The $w=m-1$ case therefore serves as a natural analytical worst-case bound: if closed-form inference is possible here, it establishes performance limits for the hardest identifiable masking scenario.
...
Despite its limitations, the uniform masking model with $w = m-1$ serves as a natural \emph{reference model} for masked failure data: it is the \emph{maximally uninformative identifiable} masking configuration, since each observation excludes only a single component from suspicion and retains the minimum diagnostic information needed for identifiability. In this sense, uniform masking at $w = m-1$ plays a role analogous to the exponential distribution in lifetime modeling---both are analytically tractable worst cases that provide baseline performance bounds, and both owe their tractability to a maximum-entropy--like property (constant hazard rate for the exponential; uniform inclusion probability for the masking model).
\end{remark}
...
\begin{remark}
In the symmetric case $\tparam{\lambda}_1 = \cdots = \tparam{\lambda}_m$, the mutual information has the explicit form $\RV{I}(\RV{K}; \cand_w) = \ln(m/w)$, which ranges from $\ln m$ (exact identification, $w=1$) through $\ln(m/(m-1))$ (maximum masking with identifiability, $w=m-1$) to $0$ (complete masking, $w=m$). Combined with the exponential lifetime assumption, the configuration $(w = m-1,\, \expdist)$ is thus doubly ``worst case'': exponential lifetimes maximize entropy among distributions with a given mean, and uniform masking at $w = m-1$ minimizes diagnostic information among identifiable masking models.
\end{remark}
...
\item For $w = m-1$---the maximally uninformative masking configuration that remains identifiable---the score equations linearize and a closed-form MLE exists for arbitrary~$m$. Combined with the exponential lifetime assumption, this yields a \emph{doubly worst-case reference model}: exponential lifetimes maximize entropy among fixed-mean distributions, while uniform masking at $w = m-1$ minimizes diagnostic information among identifiable configurations (Proposition~\ref{prop:mutual_info}). The closed-form results thus provide analytical performance bounds for the hardest identifiable masking scenario
